 ...
 * - ADDED: Remote logging to Node.js backend
 */
...
(function() {
    ...
    function initToolsRefactor() {
        ...
        Object.keys(buttonMap).forEach(function(id) {
            ...
            clone.addEventListener('click', function(e) {
                ...
                sendLog('Button Clicked (Tools Tab): ' + id);
                buttonMap[id](e);
            });
    ...
    async function handleJoinPhrases(e) {
        ...
        sendLog('Action Started: Join Phrases');
        ...
        try {
            ...
            if (result.status === 'Error') {
                ...
                sendLog('Join Phrases Failed: ' + result.message);
                alert('Join Failed\n\nDetails: ' + result.message);
            } else {
                progress.stop(true); progress = null;
                sendLog('Join Phrases Completed Successfully');
    ...
    async function handleSplitJoinSelection(e) {
        ...
        sendLog('Action Started: Split & Join Selection');
        ...
        try {
            ...
            if (result.status === 'Error') {
                progress.stop(false); progress = null;
                sendLog('Split & Join Selection Failed: ' + result.message);
                alert('Split & Join Failed\n\nDetails: ' + result.message);
            } else {
                progress.stop(true); progress = null;
                sendLog('Split & Join Selection Completed Successfully');
    ...
    async function handleSyncGeneric(e, type) {
        ...
        sendLog('Action Started: Sync (' + type + ')');
        ...
        try {
            ...
            try {
                ...
                sendLog(`Sync (${type}) Completed: synced=${synced}, skipped=${skipped}`);
                ...
                sendLog('Sync (' + type + ') Failed: ' + err.message);
    ...
    async function handleSplitPhrase(e) {
        ...
        sendLog('Action Started: Split Phrase');
        ...
        try {
            ...
            if (result.status === 'Error') {
                progress.stop(false); progress = null;
                sendLog('Split Phrase Failed: ' + result.message);
                alert('Split Failed\n\nDetails: ' + result.message);
            } else {
                progress.stop(true); progress = null;
                sendLog('Split Phrase Completed Successfully');
    ...
    async function handleRemoveWord(e) {
        sendLog('Action Started: Remove Word');
        ...
        try {
            ...
            if (result && result.status !== 'Complete') {
                sendLog('Remove Word Failed: ' + (result.message || 'Unknown error'));
                alert('Remove Word: ' + (result.message || 'Unknown error'));
            } else {
                sendLog('Remove Word Completed Successfully');
            }
        } catch(err) {
            progress.stop(false);
    ...
    async function handleAddWord(e) {
        sendLog('Action Started: Add Word');
        ...
        try {
            ...
            progress.stop(result && result.status === 'Complete');
            if (result && result.status === 'Complete') {
                sendLog('Add Word Completed Successfully: ' + newWord);
                if (newWordInput) newWordInput.value = '';  // clear input on success
            } else {
                sendLog('Add Word Failed: ' + (result ? result.message : 'Unknown error'));
