      ...
      for (var pi = 0; pi < rebuildList.length; pi++) {
        ...
        if (mgtComp && mgtComp.properties) {
          if (params.variationParams) {
            var isArr = (Object.prototype.toString.call(params.variationParams) === '[object Array]');
            for (var key in params.variationParams) {
              if (params.variationParams.hasOwnProperty(key)) {
                try {
                  var pName = isArr ? params.variationParams[key].displayName : key;
                  var pVal  = isArr ? params.variationParams[key].value : params.variationParams[key];
                  if (!pName) continue;
                  var vp = mgtComp.properties.getParamForDisplayName(pName);
                  if (vp) {
                    if (typeof _applyMogrtPropValue === "function") {
                      _applyMogrtPropValue(vp, pName, pVal);
                    } else {
                      vp.setValue(pVal, 1);
                    }
                  }
        ...
        for (var k = 0; k < variationParams.length; k++) {
          ...
          try {
            ...
            if (!vp || vp.displayName !== item.displayName) {
              // Fallback if index shifted
              vp = mgt.properties.getParamForDisplayName(item.displayName);
            }
            if (vp) {
              debugLogs.push("FOUND: " + item.displayName + " = " + item.value);
              if (typeof _applyMogrtPropValue === "function") {
                _applyMogrtPropValue(vp, item.displayName, item.value);
              } else {
                vp.setValue(item.value, 1);
              }
            } else {
              debugLogs.push("NOT FOUND: " + item.displayName);
            }
          } catch (e) {
            debugLogs.push("ERROR applying " + item.displayName + ": " + e.message);
          }
        }
      } else {
        // Fallback for old flat dictionary
        for (var key in variationParams) {
          if (variationParams.hasOwnProperty(key)) {
            try {
              var vp = mgt.properties.getParamForDisplayName(key);
              if (vp) {
                debugLogs.push("FOUND: " + key + " = " + variationParams[key]);
                if (typeof _applyMogrtPropValue === "function") {
                  _applyMogrtPropValue(vp, key, variationParams[key]);
                } else {
                  vp.setValue(variationParams[key], 1);
                }
              } else {
                debugLogs.push("NOT FOUND: " + key);
              }
            } catch (e) {
              debugLogs.push("ERROR applying " + key + ": " + e.message);
            }
          }
        }
      }
