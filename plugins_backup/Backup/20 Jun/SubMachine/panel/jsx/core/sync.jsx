    ...
    for (var k = 0; k < pCount; k++) {
      ...
      if (p.propertyValueType === 6) {
        try {
          ...
          if (co) {
            ...
            var a = aRaw != null ? Number(aRaw) : 1;
            if (r > 1 || g > 1 || b > 1) { r /= 255; g /= 255; b /= 255; }
            if (a > 1) a /= 255;
            entry = { displayName: name, kind: "color", value: [r, g, b, a] };
          }
        } catch (eC) { }
      }
      ...
      if (!entry) {
        ...
        if (typeof v === 'string' && v.charAt(0) === '{') {
          ...
          entry = { displayName: name, kind: "bool", value: v };
        } else if (typeof v === 'number') {
          entry = { displayName: name, kind: "number", value: v };
        } else if (Object.prototype.toString.call(v) === '[object Array]') {
          var arr = [];
          for (var ai = 0; ai < v.length; ai++) arr.push(v[ai]);
          entry = { displayName: name, kind: "array", value: arr };
        }
      }
...
function _applyMogrtPropValue(prop, propName, val) {
  ...
  // Guard against Group UUID strings (which corrupt the MOGRT if applied)
  if (typeof val === 'string' && val.indexOf('-') !== -1 && val.indexOf(';') !== -1) {
    if (val.match(/^[0-9a-fA-F\-;]+$/)) {
      return; // Skip group properties
    }
  }
  ...
  } else if (prop.propertyValueType === 6 && val && typeof val.length === 'number' && val.length >= 3) {
    try {
      var ca = val[0], cr = val[1], cg = val[2], cb = val[3];
      if (val.length === 3) { ca = 255; cr = val[0]; cg = val[1]; cb = val[2]; }
      if (ca === 1) ca = 255; // Default missing or 1.0 alpha to 255
      
      prop.setColorValue(ca, cr, cg, cb, 1);
    } catch (e) {
      prop.setValue([cb, cr, cg, ca], 1);
    }
  } else {
    try {
      // Support points
