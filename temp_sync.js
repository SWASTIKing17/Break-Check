  ...
  return safeCall(function () {
    ...
    for (var i = 0; i < seq.videoTracks.numTracks; i++) {
      ...
      for (var j = 0; j < trk.clips.numItems; j++) {
        ...
        if (!isMogrt) {
            try { var m = c.getMGTComponent(); if (m) isMogrt = true; } catch (e) {}
        }
        if (!isMogrt) {
            try {
                var pPath = c.projectItem.getMediaPath();
                if (pPath && pPath.toLowerCase().indexOf(".aegraphic") > -1) isMogrt = true;
            } catch(e) {}
        }
        if (!isMogrt) continue;
    ...
    var mediaPath = "";
    try { mediaPath = found.projectItem.getMediaPath() || ""; } catch (e) {}
    if (!mediaPath) {
        try { mediaPath = found.projectItem.name || ""; } catch (e) {}
    }
    if (!mediaPath) {
        try { mediaPath = found.name || ""; } catch (e) {}
    }
