...
- **Deterministic Card Tints**: Every MOGRT card is assigned a unique, vibrant Card Tint computed deterministically from its base name. The exact same tint and color badge are applied to phrase cards in **FreeXan Caption**'s Edit tab, allowing visual differentiation and instant correlation between MOGRTs and phrase tracks.
