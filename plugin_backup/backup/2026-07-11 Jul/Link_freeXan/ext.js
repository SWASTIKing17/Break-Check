...
csInterface.evalScript = function(script, callback) {
    // BUG FIX: Prevent spamming the console and node backend with the 1-second polling tracking script
    const isNoisyPoll = script.indexOf('if(!app||!app.project)return "NONE||";') !== -1 || script.indexOf('return "wait";') !== -1;
...
function connectWebSocket() {
    ...
    ws.onmessage = function(event) {
        try {
            ...
            if (data.type === 'cep_heartbeat_ping') {
                try { ws.send(JSON.stringify({ type: 'cep_heartbeat', source: 'link' })); } catch(e){}
                return;
            }
            ...
            // ── MCP / CLI plugin_action dispatcher ──────────────────────────
            // Handles actions dispatched by dispatchToPlugin('link', action, args)
            // from main.js → httpApi.js → POST /plugin-action.
            // Each action runs an inline JSX IIFE and sends plugin_action_result back.
            if (data.type === 'plugin_action') {
                var paRequestId = data.requestId;
                var paAction    = data.action;
                var paArgs      = data.args || {};

                function sendPluginResult(result) {
                    try { ws.send(JSON.stringify({ type: 'plugin_action_result', requestId: paRequestId, result: result })); } catch(e) {}
                }
                function sendPluginError(msg) {
                    try { ws.send(JSON.stringify({ type: 'plugin_action_result', requestId: paRequestId, error: msg })); } catch(e) {}
                }

                extLog('[PLUGIN_ACTION] action=' + paAction + ' requestId=' + paRequestId);

                // ── link_status — active project + sequence info ─────────────
                if (paAction === 'link_status') {
                    var statusScript = '(function(){' +
                        'try{' +
                        '  if(!app||!app.project)return JSON.stringify({connected:false,projectName:null,projectPath:null,sequenceCount:0,activeSequence:null});' +
                        '  var proj=app.project;' +
                        '  var seqs=[];' +
                        '  for(var i=0;i<proj.sequences.numSequences;i++){' +
                        '    var s=proj.sequences[i];' +
                        '    if(s)seqs.push(s.name);' +
                        '  }' +
                        ...
                        '    sequences:seqs,' +
                        '    activeSequence:activeSeq?activeSeq.name:null' +
                        '  });' +
                        '}catch(e){return JSON.stringify({connected:false,error:e.message});}' +
                        '})()';
                    csInterface.evalScript(statusScript, function(r) {
                        try { sendPluginResult(JSON.parse(r)); } catch(e) { sendPluginResult({ raw: r }); }
                    });
                    return;
                }

                // ── link_list_bins — recursive bin tree ──────────────────────
                if (paAction === 'link_list_bins') {
                    var listBinsScript = '(function(){' +
                        'try{' +
                        '  if(!app||!app.project||!app.project.rootItem)return JSON.stringify({err:"no project"});' +
                        '  function collectBins(parent,depth){' +
                        '    var out=[];' +
                        '    for(var i=0;i<2000;i++){' +
                        '      var c=parent.children[i];if(!c)break;' +
                        '      if(c.type===2){' +
                        '        out.push({name:c.name, depth:depth, children:collectBins(c,depth+1)});' +
                        '      }' +
                        '    }' +
                        '    return out;' +
                        '  }' +
                        '  return JSON.stringify({bins: collectBins(app.project.rootItem,0)});' +
                        '}catch(e){return JSON.stringify({err:e.message});}' +
                        '})()';
                    csInterface.evalScript(listBinsScript, function(r) {
                        try { sendPluginResult(JSON.parse(r)); } catch(e) { sendPluginResult({ raw: r }); }
                    });
                    return;
                }

                // ── link_create_bin — create a bin at an optional pipe-delimited parent path ──
                if (paAction === 'link_create_bin') {
                    if (!paArgs.name) { sendPluginError('link_create_bin: missing args.name'); return; }
                    var cbName = String(paArgs.name).replace(/\\/g, '\\\\').replace(/"/g, '\\"');
                    // parentPath is pipe-delimited, e.g. "02_Footage|B-Roll"
                    var cbParent = paArgs.parentPath ? String(paArgs.parentPath).replace(/\\/g, '\\\\').replace(/"/g, '\\"') : '';
                    var createBinScript = '(function(){' +
                        'try{' +
                        '  if(!app||!app.project||!app.project.rootItem)return JSON.stringify({err:"no project"});' +
                        '  var p=app.project.rootItem;' +
                        '  var parentStr="' + cbParent + '";' +
                        '  if(parentStr.length>0){' +
                        '    var pts=parentStr.split("|");' +
                        '    for(var i=0;i<pts.length;i++){' +
                        '      var found=false;' +
                        '      for(var j=0;j<2000;j++){var c=p.children[j];if(!c)continue;if(c.name===pts[i]&&c.type===2){p=c;found=true;break;}}' +
                        '      if(!found)return JSON.stringify({err:"parent bin not found: "+pts[i]});' +
                        '    }' +
                        '  }' +
                        '  p.createBin("' + cbName + '");' +
                        '  return JSON.stringify({ok:true,name:"' + cbName + '",parentPath:"' + cbParent + '"});' +
                        '}catch(e){return JSON.stringify({err:e.message});}' +
                        '})()';
                    csInterface.evalScript(createBinScript, function(r) {
                        extLog('[PLUGIN_ACTION] link_create_bin → ' + r);
                        try { sendPluginResult(JSON.parse(r)); } catch(e) { sendPluginResult({ raw: r }); }
                    });
                    return;
                }

                // ── link_create_sequence — create a named sequence with optional preset ──
                if (paAction === 'link_create_sequence') {
                    if (!paArgs.name) { sendPluginError('link_create_sequence: missing args.name'); return; }
                    var csName = String(paArgs.name).replace(/\\/g, '\\\\').replace(/"/g, '\\"');
                    // Use named preset from sqpersets/ if provided, otherwise fall back to default
                    var eExtPath = extensionPath.replace(/\\/g, '\\\\');
                    var csPreset = paArgs.preset
                        ? eExtPath + '\\\\sqpersets\\\\' + String(paArgs.preset) + '.sqpreset'
                        : eExtPath + '\\\\sequence-preset.sqpreset';
                    csPreset = csPreset.replace(/"/g, '\\"');
                    var eSPath = paArgs.parentPath ? String(paArgs.parentPath).replace(/\\/g, '\\\\').replace(/"/g, '\\"') : '';
                    
                    var createSeqScript = '(function(){' +
                        'try{' +
                        '  if(!app||!app.project)return JSON.stringify({err:"no project"});' +
                        '  var pPath="' + csPreset + '";' +
                        '  var sName="' + csName + '";' +
                        '  var method="none";' +
                        ...
                        '    try{app.enableQE();if(typeof qe!=="undefined"&&qe.project&&typeof qe.project.newSequence==="function"){qe.project.newSequence(sName,pPath);method="qe";}}catch(e2){}' +
                        '  }' +
                        '  if(method==="none"||method.indexOf("fail")!==-1){' +
                        '    app.project.createNewSequence(sName,"");method="dialog";' +
                        '  }' +
                        '  return JSON.stringify({ok:true,name:sName,method:method,parentPath:"'+eSPath+'"});' +
                        '}catch(e){return JSON.stringify({err:e.message});}' +
                        '})()';
                        
                    csInterface.evalScript(createSeqScript, function(r) {
                        extLog('[PLUGIN_ACTION] link_create_sequence → ' + r);
                        var parsed = {raw: r};
                        try { parsed = JSON.parse(r); } catch(e) {}
                        
                        if (parsed.ok && parsed.parentPath && parsed.parentPath.length > 0) {
                            setTimeout(function() {
                                var moveScript = '(function(){' +
                                    'try{' +
                                    '  var sName="' + parsed.name + '";' +
                                    '  var eSPath="' + parsed.parentPath + '";' +
                                    '  var seqItem=null;' +
                                    '  for(var k=0;k<2000;k++){var item=app.project.rootItem.children[k];if(!item)continue;if(item.name===sName&&item.type!==2){seqItem=item;break;}}' +
                                    '  if(!seqItem)return "err:seq item not found in root";' +
                                    '  var pts=eSPath.split("|");var tgt=app.project.rootItem;' +
                                    '  for(var i=0;i<pts.length;i++){var found=false;for(var j=0;j<2000;j++){var c=tgt.children[j];if(!c)continue;if(c.name===pts[i]&&c.type===2){tgt=c;found=true;break;}}' +
                                    '  if(!found)return "err:target bin not found: "+pts[i];}' +
                                    '  seqItem.moveBin(tgt);' +
                                    '  return "ok";' +
                                    '}catch(e){return "err:"+e.message;}' +
                                    '})()';
                                csInterface.evalScript(moveScript, function(mr) {
                                    if(mr !== "ok") parsed.moveErr = mr;
                                    sendPluginResult(parsed);
                                });
                            }, 500); // 500ms delay to allow Premiere to register the new sequence
                        } else {
                            sendPluginResult(parsed);
                        }
                    });
                ...
                // Unknown action — send error so dispatchToPlugin resolves rather than times out
                sendPluginError('link: unknown action "' + paAction + '"');
                return;
            }
            // ── End MCP / CLI plugin_action dispatcher ───────────────────────
        } catch(e) {
