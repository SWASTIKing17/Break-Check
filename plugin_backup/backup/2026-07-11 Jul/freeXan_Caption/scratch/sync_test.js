...
function syncAll(data) {
  return safeCall(function () {
    ...
    // F: Motion (Position/Scale/Rotation) is IGNORED for Sync All per standard §8A.
    // It is only applied when masterPositionValue is set — which the frontend now
    // clears for all types except PSR.

    // Sync MOGRT properties — match by exact Path (group-aware)
    var mgt = clip.getMGTComponent();
    for (var k = 0; k < updatedProps.length; k++) {
      var item = updatedProps[k];
      var pathString = item.path ? item.path.join(" > ") : item.displayName;

      // H: Backend Progression guard
      if (item.displayName.indexOf("Progression") !== -1) continue;

      // Glyph-Based Sync Guard (Ⓢ and Ⓑ)
      var hasSpecificGlyph = (pathString.indexOf("\u24c8") !== -1 || pathString.indexOf("\u24b7") !== -1 || pathString.indexOf("Ⓢ") !== -1 || pathString.indexOf("Ⓑ") !== -1);
      if (hasSpecificGlyph && !isSamePhrase) {
        jsxLog("  [GUARD] Skipping phrase-specific: " + pathString, "INFO");
        continue;
      }
