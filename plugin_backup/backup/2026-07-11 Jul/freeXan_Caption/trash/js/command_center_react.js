...
const App = () => {
    ...
    const handleReplaceMogrt = async (externalFilePath = null, externalParams = null) => {
        ...
        let mogrtFilePath = (typeof externalFilePath === 'string') ? externalFilePath : null;
        if (!mogrtFilePath) {
            try {
                const csInterface = new window.CSInterface();
                const path = window.require('path');
                const fs = window.require('fs');
                const fxDir = path.join(csInterface.getSystemPath(window.SystemPath.USER_DATA), 'freeXan');
                const tempFile = path.join(fxDir, 'active_mogrt.txt');
                if (fs.existsSync(tempFile)) {
                    const latestMogrt = fs.readFileSync(tempFile, 'utf8').trim();
                    if (latestMogrt) {
                        mogrtFilePath = latestMogrt;
                        const mogrtInput = document.getElementById('mogrtFile');
                        if (mogrtInput) {
                            mogrtInput.value = mogrtFilePath;
                            mogrtInput.dispatchEvent(new Event('change'));
                        }
                    }
                }
            } catch (e) {
                console.error("Could not read active MOGRT from BloomX in CC:", e);
            }
        }
        if (!mogrtFilePath) {
            const mogrtInput = document.getElementById('mogrtFile');
