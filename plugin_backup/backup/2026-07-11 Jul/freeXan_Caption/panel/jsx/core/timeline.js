...
function moveClipToTrack(clip, sourceTrackIdx, destTrackIdx, optionalProjectItem) {
  if (destTrackIdx < 0) {
  ...
  var projectItem    = optionalProjectItem || clip.projectItem;

  // Bail out if track is the same AND template is exactly the same
  if (sourceTrackIdx === destTrackIdx && projectItem.nodeId === clip.projectItem.nodeId) return clip;
  ...
  return safeCall(function () {
    ...
    for (var g = 0; g < discoveredGroups.length; g++) {
      ...
      for (var c = 0; c < grp.clips.length; c++) {
        ...
        var moved = moveClipToTrack(origClip, grp.trackIdx, safeTrackIdx, masterClip.projectItem);
...
function sm_tools_split_join_v28(params) {
  return safeCall(function () {
    ...
    var mergedWords = before ? stolenWords.concat(targetWords) : targetWords.concat(stolenWords);
    var mergedIdxs = [];
    if (before) {
        // Stolen words at start adapt the first word's style of target phrase
        for (var i = 0; i < stolenWords.length; i++) mergedIdxs.push(0);
        for (var i = 0; i < targetWords.length; i++) mergedIdxs.push(i);
    } else {
        // Stolen words at end adapt the last word's style of target phrase
        for (var i = 0; i < targetWords.length; i++) mergedIdxs.push(i);
        var lastIdx = targetWords.length > 0 ? targetWords.length - 1 : 0;
        for (var i = 0; i < stolenWords.length; i++) mergedIdxs.push(lastIdx);
    }
    
    var mergedTextObj = buildTextObj(_tgtTObj, mergedWords, mergedIdxs);
    ...
    var rpi = remainderClips.length > 0 ? remainderClips[0].projectItem : null;
    for (var rc = 0; rc < remainderClips.length; rc++) {
      var remTrkIdx = _getClipTrackIndex(remainderClips[rc]);
      if (remTrkIdx >= 0) {
        var movedR = moveClipToTrack(remainderClips[rc], remTrkIdx, safeTrackRemainder, rpi);
        if (movedR) remainderClips[rc] = movedR;
      }
    }

    // --- Pass B: Move MERGED clips to their destination track first ---
    var mpi = targetPhrase.length > 0 ? targetPhrase[0].projectItem : null;
    for (var n = 0; n < mergedClips.length; n++) {
      var mrgTrkIdx = _getClipTrackIndex(mergedClips[n]);
      if (mrgTrkIdx >= 0) {
        var movedM = moveClipToTrack(mergedClips[n], mrgTrkIdx, safeTrackMerged, mpi);
...
function sm_tools_remove_word_v28(params) {
  return safeCall(function() {
    jsxLog("Executing Fast Remove Word on T" + params.trackIndex + " C" + params.clipIndex, "INFO");
    var activeSeq = app.project.activeSequence;
    if (!activeSeq) throw new Error("No active sequence.");
    ...
    var rawWords = (textObj && typeof textObj.textEditValue === 'string')
      ? textObj.textEditValue.split(/\s+/)
      : [];
    var words = [];
    for (var wIdx = 0; wIdx < rawWords.length; wIdx++) {
      if (rawWords[wIdx] !== "") words.push(rawWords[wIdx]);
    }
    ...
    for (var ni = 0; ni < words.length; ni++) {
      if (ni < targetPos) newIndices.push(ni);
      else newIndices.push(ni + 1);
    }
    var newTextObj = buildTextObj(textObj, words, newIndices);
    var newTextObjStr = JSON.stringify(newTextObj);
    ...
    try {
      ...
      // 5. Delete the target clip
      try { targetClip.remove(false, false); } catch(e) { jsxLog("remove failed: " + e.toString(), "ERROR"); }

      // --- P1 PRIORITY OPTIMIZATION: UPDATE MOGRT UNDER PLAYHEAD IMMEDIATELY ---
      if (instantIdx !== -1 && phraseClips[instantIdx] && phraseClips[instantIdx].isMGT && phraseClips[instantIdx].isMGT()) {
        var iMgt = phraseClips[instantIdx].getMGTComponent();
        var iTxt = iMgt.properties.getParamForDisplayName("Ⓣ Text Input") ||
                   iMgt.properties.getParamForDisplayName("Text Input");
        if (iTxt) iTxt.setValue(newTextObjStr, 0);

        if (targetPos === 0) {
          var iProg = iMgt.properties.getParamForDisplayName("Ⓣ Word Progression") ||
                      iMgt.properties.getParamForDisplayName("Word Progression");
          if (iProg) iProg.setValue(1, 0);
        }
      }

      // 6. Shift progression ONLY for clips AFTER removed position! Do NOT touch Text Input (zero UI lag)
      for (var k = targetPos + 1; k < phraseClips.length; k++) {
        if (k === instantIdx) continue;
        var c = phraseClips[k];
        if (!c || !c.isMGT || !c.isMGT()) continue;
        ...
        if (cProg) cProg.setValue(k, 0);
      }

      jsxLog("Fast Remove Word DONE | removed pos=" + targetPos + " | new text='" + newText + "'", "INFO");
      return { status: "Complete" };
...
// ==========================================
// TOOL 5: ADD WORD (Fast Duplicate Model)
// ==========================================
...
function sm_tools_add_word_v28(params) {
  return safeCall(function() {
    ...
    // 2. Extract Text Input and Master Style
    var anchorMgt = anchorClip.getMGTComponent();
    var textParam = anchorMgt.properties.getParamForDisplayName("Ⓣ Text Input") || 
                    anchorMgt.properties.getParamForDisplayName("Text Input") ||
                    anchorMgt.properties.getParamForDisplayName("Ⓣ Text Input");
    var textObj   = JSON.parse(textParam.getValue());
    var rawWords  = textObj.textEditValue.split(/\s+/);
    var words = [];
    for (var aIdx = 0; aIdx < rawWords.length; aIdx++) {
      if (rawWords[aIdx] !== "") words.push(rawWords[aIdx]);
    }
    var masterStyleMap = extractMasterStyle(anchorClip);
    var projectItem = anchorClip.projectItem;

    // 3. Build new phrase text: insert newWord after anchorPos
    var newWords = words.slice();
    newWords.splice(anchorPos + 1, 0, newWord);
    var newIndices = [];
    for (var ni = 0; ni < newWords.length; ni++) {
      if (ni <= anchorPos) newIndices.push(ni);
      else if (ni === anchorPos + 1) newIndices.push(anchorPos);
      else newIndices.push(ni - 1);
    }
    ...
    startUndo("Add Word");
    try {
      // 4. Trim anchor clip (Clip A) to end at playhead
      var tEnd = new Time(); tEnd.seconds = playhead;
      anchorClip.end = tEnd;

      // 5. Overwrite new clip instance (Clip B) starting at playhead
      track.overwriteClip(projectItem, playhead);
      var clipB = null;
      for (var cb = track.clips.numItems - 1; cb >= 0; cb--) {
        if (Math.abs(track.clips[cb].start.seconds - playhead) < 0.01) {
          clipB = track.clips[cb];
          break;
        }
      }
      if (!clipB) return { status: "Error", message: "Could not locate clip B after overwrite." };
      var bEnd = new Time(); bEnd.seconds = anchorEnd;
      clipB.end = bEnd;

      // 6. Apply master style ONLY to Clip B
      applyMasterStyle(clipB, masterStyleMap);

      // --- P1 PRIORITY OPTIMIZATION: UPDATE MOGRTS UNDER PLAYHEAD IMMEDIATELY ---
      var bMgt = clipB.getMGTComponent();
      if (bMgt) {
        var bTxt = bMgt.properties.getParamForDisplayName("Ⓣ Text Input") || 
                   bMgt.properties.getParamForDisplayName("Text Input");
        if (bTxt) bTxt.setValue(newTextObjStr, 0);
        var bProg = bMgt.properties.getParamForDisplayName("Ⓣ Word Progression") || 
                    bMgt.properties.getParamForDisplayName("Word Progression");
        if (bProg) bProg.setValue(anchorPos + 2, 0);
      }
      var aMgt = anchorClip.getMGTComponent();
      if (aMgt) {
        var aTxt = aMgt.properties.getParamForDisplayName("Ⓣ Text Input") || 
                   aMgt.properties.getParamForDisplayName("Text Input");
        if (aTxt) aTxt.setValue(newTextObjStr, 0);
      }

      // 7. Update Progression ONLY for clips AFTER inserted clip! Do NOT touch Text Input (zero UI lag)
      for (var k = anchorPos + 1; k < phraseClips.length; k++) {
        var c = phraseClips[k];
        if (!c || !c.isMGT || !c.isMGT()) continue;
        var cMgt = c.getMGTComponent();
        var cProg = cMgt.properties.getParamForDisplayName("Ⓣ Word Progression") || 
                    cMgt.properties.getParamForDisplayName("Word Progression");
        if (cProg) cProg.setValue(k + 2, 0);
      }

      jsxLog("Fast Add Word DONE | inserted='" + newWord + "' | playhead=" + playhead.toFixed(3) + "s", "INFO");
      return { status: "Complete" };
...
// ==========================================
// TOOL 6: RESET WORD PROGRESSION
function sm_tools_reset_progression_v28(params) {
  return safeCall(function() {
    var activeSeq = app.project.activeSequence;
    if (!activeSeq) throw new Error("No active sequence.");
    ...
    var mogrts = [];
    var sel = null;
    try { sel = activeSeq.getSelection(); } catch(e) {}
    
    if (sel && sel.length > 0) {
      for (var i = 0; i < sel.length; i++) {
        if (sel[i] && sel[i].isMGT && sel[i].isMGT()) mogrts.push(sel[i]);
      }
    }
    
    // Fallback if getSelection() threw DOM error or returned nothing
    if (mogrts.length === 0) {
      for (var t = 0; t < activeSeq.videoTracks.numTracks; t++) {
        var tr = activeSeq.videoTracks[t];
        for (var c = 0; c < tr.clips.numItems; c++) {
          var cl = tr.clips[c];
          try {
            if (cl.isSelected() && cl.isMGT && cl.isMGT()) mogrts.push(cl);
          } catch(_) {}
        }
      }
    }
    ...
    if (mogrts.length === 0) {
      return { status: "Error", message: "No caption MOGRTs found in selection." };
    }
    mogrts.sort(function(a, b) { return a.start.seconds - b.start.seconds; });
    
    startUndo("Reset Word Progression");
    try {
      for (var k = 0; k < mogrts.length; k++) {
        var mgt = mogrts[k].getMGTComponent();
        var prog = mgt.properties.getParamForDisplayName("Ⓣ Word Progression") ||
                   mgt.properties.getParamForDisplayName("Word Progression") ||
                   mgt.properties.getParamForDisplayName("Ⓣ Word Progression");
        if (prog) prog.setValue(k + 1, 0);
      }
      jsxLog("Reset Progression DONE for " + mogrts.length + " clips.", "INFO");
      return { status: "Complete", count: mogrts.length };
    } finally {
      endUndo("Reset Word Progression");
    }
  }, "sm_tools_reset_progression_v28");
}
...
// ==========================================
// TOOL: TEXT FORMATTING (SELECTED MOGRTS)
// ==========================================
function sm_tools_format_text_selected_mogrts(params) {
  return safeCall(function () {
    app.enableQE();
    var formatType = params.formatType || "sentence";
    jsxLog("Executing Text Format (" + formatType + ") for " + params.selectedClips.length + " clips", "INFO");
    var activeSeq = app.project.activeSequence;
    if (!activeSeq) throw new Error("No active sequence.");

    startUndo("Format Text MOGRTs");
    try {
      var selectedClipsData = params.selectedClips;
      for (var i = 0; i < selectedClipsData.length; i++) {
        var pfD = selectedClipsData[i];
        var pfTIdx = parseInt(pfD.trackIndex, 10);
        var pfCIdx = parseInt(pfD.clipIndex, 10);
        if (isNaN(pfTIdx) || isNaN(pfCIdx)) continue;
        
        var track = activeSeq.videoTracks[pfTIdx];
        if (!track) continue;
        var clip = track.clips[pfCIdx];
        if (!clip || !clip.isMGT()) continue;

        var mgt = clip.getMGTComponent();
        if (!mgt) continue;
        
        var textParamNames = ["Ⓣ Text Input", "\u24c9 Text Input", "Ⓢ Text Input", "Text Input", "Text", "Source Text"];
        var textParam = null;
        for (var n = 0; n < textParamNames.length; n++) {
            textParam = mgt.properties.getParamForDisplayName(textParamNames[n]);
            if (textParam) break;
        }
        if (!textParam) continue;

        var rawStr = textParam.getValue();
        if (typeof rawStr !== 'string') continue;
        
        var parsed = {};
        try { parsed = JSON.parse(rawStr); } catch (e) { continue; }
        
        if (parsed && typeof parsed.textEditValue === 'string') {
          var originalText = parsed.textEditValue;
          if (originalText.length > 0) {
            var leadingSpaceMatch = originalText.match(/^\s*/);
            var trailingSpaceMatch = originalText.match(/\s*$/);
            var leadingSpaces = leadingSpaceMatch ? leadingSpaceMatch[0] : "";
            var trailingSpaces = trailingSpaceMatch ? trailingSpaceMatch[0] : "";
            
            var trimmedText = originalText.substring(leadingSpaces.length, originalText.length - trailingSpaces.length);
            ...
            if (trimmedText.length > 0) {
              ...
              switch(formatType) {
                case "uppercase":
                  newText = trimmedText.toUpperCase();
                  break;
                case "lowercase":
                  newText = trimmedText.toLowerCase();
                  break;
                case "sentence":
                  newText = trimmedText.charAt(0).toUpperCase() + trimmedText.slice(1).toLowerCase();
                  break;
                case "title":
                  for (var w = 0; w < words.length; w++) {
                    if (words[w].length > 0) {
                      words[w] = words[w].charAt(0).toUpperCase() + words[w].slice(1).toLowerCase();
                    }
                  }
                  newText = words.join(" ");
                  break;
                case "first_upper":
                  if (words.length > 0) {
                    ...
                    for (var w = 1; w < words.length; w++) words[w] = words[w].toLowerCase();
                    newText = words.join(" ");
                  }
                  break;
                case "last_upper":
                  if (words.length > 0) {
                    for (var w = 0; w < words.length - 1; w++) words[w] = words[w].toLowerCase();
                ...
                case "alternating":
                  var alt = "";
                  var isUpper = true;
                  for (var c = 0; c < trimmedText.length; c++) {
                    var charCode = trimmedText.charCodeAt(c);
                    if ((charCode >= 65 && charCode <= 90) || (charCode >= 97 && charCode <= 122)) {
                      alt += isUpper ? trimmedText.charAt(c).toUpperCase() : trimmedText.charAt(c).toLowerCase();
                      isUpper = !isUpper;
                    } else {
                      alt += trimmedText.charAt(c);
                    }
                  }
                  newText = alt;
                  break;
                case "snake_case":
                  newText = trimmedText.toLowerCase().replace(/\s+/g, '_');
                  break;
                case "camelcase":
                  if (words.length > 0) {
                    var camel = words[0].toLowerCase();
                    for (var w = 1; w < words.length; w++) {
                      camel += words[w].charAt(0).toUpperCase() + words[w].slice(1).toLowerCase();
                    }
                    newText = camel;
                  }
                  break;
                case "vowel":
                  ...
                  for (var c = 0; c < trimmedText.length; c++) {
                    var cChar = trimmedText.charAt(c);
                    if (vowels.indexOf(cChar) !== -1) {
                      vwl += cChar.toUpperCase();
                    } else {
                      vwl += cChar.toLowerCase();
                    }
                  }
                  newText = vwl;
                  break;
              ...
              var finalText = leadingSpaces + newText + trailingSpaces;
              parsed.textEditValue = finalText;
              parsed.fontTextRunLength = [finalText.length];
              textParam.setValue(JSON.stringify(parsed), 0);
            }
          }
        }
      }
      ...
      endUndo("Format Text MOGRTs");
    }
  }, "sm_tools_format_text_selected_mogrts");
}
