...
/**
 * Specialized logger for MOGRT parameter fetching diagnosis.
 * Writes detailed inspection of every property on MOGRT clips to understand why
 * Text Input or Word Progression might be missed.
 */
function paramFetchLog(msg, level) {
  level = level || "INFO";
  var timestamp = new Date().toTimeString().split(' ')[0];
  var logMsg = "[" + timestamp + "] [" + level + "] " + msg;
  $.writeln(logMsg);
  try {
    var logFile = new File(
      Folder.userData.fsName +
      "/Adobe/CEP/extensions/com.bloomx.freexan.caption/panel/logs/mogrt_param_fetch.log"
    );
    var logFolder = new Folder(logFile.parent);
    if (!logFolder.exists) logFolder.create();
    if (logFile.open("a")) {
  ...
  try {
    var fallbackFile = new File(Folder.temp.fsName + "/freexan_mogrt_param_fetch.log");
    if (fallbackFile.open("a")) {
      fallbackFile.writeln(logMsg);
      fallbackFile.close();
    }
