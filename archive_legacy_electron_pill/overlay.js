...
window.addEventListener('DOMContentLoaded', () => {
    ...
    async function pickHaloBubble(shortcut) {
        ...
        const cid = crypto.randomUUID ? crypto.randomUUID() : 'halo-' + Date.now();
        const msg = `halo pick: key ${shortcut} → ${link.folderPath} ${moveMode ? '(MOVE)' : '(COPY)'}`;
        console.log('[Overlay]', msg);
        if (window.api && window.api.overlayLog) window.api.overlayLog(msg);
        if (window.freeXanLog) window.freeXanLog('info', 'ui:halo-pick', cid, { shortcut, target: link.folderPath, moveMode, optimistic: true });
        teardownPicker();
    ...
    function cancelHaloPicker(reason) {
        ...
        console.log('[Overlay]', msg);
        if (window.api && window.api.overlayLog) window.api.overlayLog(msg);
        if (window.freeXanLog) window.freeXanLog('info', 'ui:halo-cancel', cid, { reason });
        teardownPicker();
        if (window.api && window.api.log) window.api.log('drag_n_drop', `Halo Picker Route Selected: ${routeToFolder} (moveMode: ${moveMode})`);
