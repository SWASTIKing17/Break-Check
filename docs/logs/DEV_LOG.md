...
[2026-07-11] Performance Overhaul, SQLite Mock DB & Date Picker Pagination (`app.js`, `data.js`, `server.js`, `populate_local_test_data.py`, `usage_monitor.py`): Replaced the massive client-side load loops with single-day paginated requests using a Date Picker input added to the sidebar footer. Refactored APIs to extract the `date` parameter and query only records matching the target date, reducing payload sizes to <100KB. Migrated Supabase `timestamp` column from `TEXT` to `TIMESTAMPTZ` and added composite indexes to accelerate queries. Rewrote the local mock seeder (`populate_local_test_data.py`) to connect directly to the SQLite `dashboard.db` file, seeding full telemetry (RAM, modifier keys, scroll, window titles) for the current date, allowing developers to manually query and edit data. Fixed a tracker bug in `usage_monitor.py` that captured the tracker's memory instead of the OS total system pressure, and added an adaptive sync interval (idle debouncing) to reduce backend database requests during idle periods.

## Version: v3.8.37
[2026-07-11] Dashboard QoL Polishing (`app.js`, `style.css`, `index.html`): Implemented the final UI/UX requirements from the v3 dashboard spec. Added a `.is-loading` CSS shimmer effect to all chart cards that automatically toggles during the `fetchData` Promise resolution. Added a `.mobile-toggle` hamburger button and a `@media (max-width: 1024px)` breakpoint to collapse the sidebar off-canvas for laptop screens, toggled via `toggleSidebar()`. Upgraded the "Activity Flow" chart into a Mixed Chart: active minutes are now rendered as 100%-height solid bars colored dynamically by the dominant app for that minute, while KPM (Keystrokes Per Minute) continues to render as an overlay line chart.
