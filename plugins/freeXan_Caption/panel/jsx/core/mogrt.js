 ...
 */
function _findAllTextParams(properties) {
  var found = [];
...
/**
 * _getMogrtText(textParam) — reads text from a MOGRT text parameter.
 * Returns "" on any error.
 */
function _getMogrtText(textParam) {
  if (!textParam) return "";
  try {
...
/**
 * _setMogrtText(textParam, text) — writes text to a MOGRT text parameter.
 ...
 */
function _setMogrtText(textParam, text) {
...
/**
 * _detectCapabilities(properties) — inspects a MOGRT properties collection
 * and returns the operating mode + text-input metadata.
 */
function _detectCapabilities(properties) {
  var result = {
    mogrtMode: "generic",
    textInputCount: 0,
    textInputNames: [],
  ...
  if (result.mogrtMode === "freexan") {
    ...
    var textParams = _findAllTextParams(properties);
...
/**
 * _distributeWords(wordCount, numInputs) — evenly splits word indices into
 * `numInputs` buckets. First `wordCount%numInputs` buckets get one extra.
 */
function _distributeWords(wordCount, numInputs) {
...
/**
 * _readWordTimings(projectItem) — reads the freeXan Caption_WordTimings XMP field.
 * Returns the parsed object, or null if missing/invalid.
 */
function _readWordTimings(projectItem) {
  ...
  try {
    ...
    jsxLog("_readWordTimings exception: " + e.toString(), "WARN");
    return null;
  }
}

/**
 * _writeWordTimings(projectItem, data) — serialises and writes to XMP.
 */
function _writeWordTimings(projectItem, data) {
  ...
  try {
    ...
    jsxLog("_writeWordTimings exception: " + e.toString(), "WARN");
    return false;
  }
}

/**
 * _isGenericClip(clip) — returns true if the clip has NO Word Progression
...
function getData(requestData) {
  ...
  try {
    ...
    if (activeSeq) {
      ...
      try {
        caps = _detectCapabilities(mgtProperties);
  ...
  try {
    ...
    if (captionData.mogrtMode === "generic") {
      ...
      if (mgtCompG && mgtCompG.properties) {
        textParamsG = _findAllTextParams(mgtCompG.properties);
      }
      ...
      var distribution = _distributeWords(wordCount, numInputs);
      ...
      for (var ii = 0; ii < numInputs; ii++) {
        ...
        try {
          _setMogrtText(textParamsG[ii].param, joined);
        } catch (eSetG) {
      ...
      try {
        ...
        _writeWordTimings(placedClipG.projectItem, xmpData);
    ...
    if (mgtComp && mgtComp.properties) {
      textParamsSm = _findAllTextParams(mgtComp.properties);
    }
    if (textParamsSm.length > 0) {
      ...
      _setMogrtText(firstParam, fullPhrase);
...
// =============================================================================
// SECTION 2.5 — runCaptionGenerate (v3.5.3 — Phase 2 of plugin MCP)
//
...
//   }
// =============================================================================
function runCaptionGenerate(args) {
  try {
    ...
    var phrasingSrtPath = args.phrasingSrtPath;
    ...
    jsxLog("runCaptionGenerate: start — srt=" + hinglishSrtPath + " mogrt=" + mogrtPath + " phrasingSrt=" + (phrasingSrtPath || "none") + " phraseLimit=" + charsPerPhrase, "INFO");
    ...
    for (var bi = 0; bi < blocks.length; bi++) {
      ...
      var textLines = [];
      for (var tl = tsIdx + 1; tl < lines.length; tl++) {
        textLines.push(lines[tl]);
      }
      var text = _rcwTrim(textLines.join(" "));
      if (!text) continue;
      ...
      if (wordMatches && wordMatches.length > 1) {
        ...
        for (var w = 0; w < wordMatches.length; w++) {
          totalChars += wordMatches[w].length;
        }
        var charDur = (blockEnd - blockStart) / (totalChars > 0 ? totalChars : 1);
        var currStart = blockStart;

        for (var w2 = 0; w2 < wordMatches.length; w2++) {
          var wLen = wordMatches[w2].length;
          var wDur = charDur * wLen;
          wordsList.push({
            wordText:          wordMatches[w2],
            wordDuration:      wDur,
            characterDuration: charDur,
            wordCharacters:    wLen,
            wordStart:         currStart,
            wordEnd:           currStart + wDur
          });
          currStart += wDur;
        }
      } else {
        var singleCharDur = (blockEnd - blockStart) / (text.length > 0 ? text.length : 1);
        wordsList.push({
          wordText:          text,
          wordDuration:      blockEnd - blockStart,
          characterDuration: singleCharDur,
    ...
    jsxLog("runCaptionGenerate: parsed " + wordsList.length + " word(s) from SRT", "INFO");
    ...
    if (phrasingSrtPath) {
      var pFile = new File(phrasingSrtPath);
      if (pFile.exists) {
        ...
        pFile.close();
        if (pRaw) {
          var pBlocks = _rcwTrim(pRaw).split(/\r?\n\r?\n/);
          var phraseBlocks = [];
          for (var pbi = 0; pbi < pBlocks.length; pbi++) {
            var pLinesRaw = pBlocks[pbi].split(/\r?\n/);
            var pLines = [];
            for (var pln = 0; pln < pLinesRaw.length; pln++) {
              var tpl = _rcwTrim(pLinesRaw[pln]);
              if (tpl) pLines.push(tpl);
            }
            if (pLines.length < 2) continue;
            var ptsLine = null;
            var ptsIdx = -1;
            for (var pli = 0; pli < pLines.length; pli++) {
              if (pLines[pli].indexOf("-->") !== -1) { ptsLine = pLines[pli]; ptsIdx = pli; break; }
            }
            if (!ptsLine) continue;
            var ptsMatch = ptsLine.match(/(\d{2}:\d{2}:\d{2}[,\.]\d{3})\s*-->\s*(\d{2}:\d{2}:\d{2}[,\.]\d{3})/);
            if (!ptsMatch) continue;
            var pStart = _rcwTsToMs(ptsMatch[1]) / 1000;
            var pEnd   = _rcwTsToMs(ptsMatch[2]) / 1000;
            var txtArr = [];
            for (var tx = ptsIdx + 1; tx < pLines.length; tx++) txtArr.push(pLines[tx]);
            var pbText = _rcwTrim(txtArr.join(" "));
            if (pbText) phraseBlocks.push({ start: pStart, end: pEnd, text: pbText });
          }
          ...
          if (phraseBlocks.length > 0) {
            ...
            for (var idx = 0; idx < wordsList.length; idx++) {
              ...
              var bestIdx = lastBlockIdx;
              var maxOverlap = -999999;
              for (var bIdx = lastBlockIdx; bIdx < phraseBlocks.length; bIdx++) {
                var pb = phraseBlocks[bIdx];
                var overlap = Math.min(d.wordEnd, pb.end) - Math.max(d.wordStart, pb.start);
              ...
              var pb = phraseBlocks[bestIdx];
              if (!blockWordCounts[bestIdx]) blockWordCounts[bestIdx] = 0;
              blockWordCounts[bestIdx]++;
              var wCount = blockWordCounts[bestIdx];

              d.phraseText = pb.text;
              d.phraseNumber = bestIdx + 1;
              d.numWords = wCount;
              d.progressionValue = wCount;
              d.videoTrack = (bestIdx % 2 === 0) ? 1 : 2;
            }
            for (var pbk = 0; pbk < phraseBlocks.length; pbk++) phrases.push(phraseBlocks[pbk].text);
          }
        }
      }
    }
    ...
    jsxLog("runCaptionGenerate: built " + phrases.length + " phrase(s)", "INFO");
    ...
    // Bridge any micro 1-2 frame gaps between created clips
    try { bridgeCaptionGaps(); } catch (e) { jsxLog("runCaptionGenerate: bridgeCaptionGaps error: " + e, "WARN"); }

    jsxLog("runCaptionGenerate: DONE — created=" + clipsCreated +
    ...
    return reportError(err, "runCaptionGenerate");
  }
}

// runCaptionGenerate helpers — `_rcw` prefix avoids clashing with any future
...
function replacePhraseWithMogrt(params) {
  try {
    ...
    var oldIsGeneric = _isGenericClip(firstClip);
    ...
    if (oldMode === "freexan") {
      ...
      for (var ci = 0; ci < phraseClips.length; ci++) {
        ...
        if (mgt && mgt.properties) {
          ...
          if (exactTextParam) {
            wordText = _getMogrtText(exactTextParam);
          } else {
            var sTextParams = _findAllTextParams(mgt.properties);
            if (sTextParams.length > 0) wordText = _getMogrtText(sTextParams[0].param);
          }
          var progP = null;
      ...
      var xmpData = null;
      try { xmpData = _readWordTimings(gClip.projectItem); } catch (eX) { xmpData = null; }
      ...
      if (!xmpData || !xmpData.words || xmpData.words.length === 0) {
        ...
        if (srcMgt && srcMgt.properties) {
          var srcParams = _findAllTextParams(srcMgt.properties);
          var parts = [];
          for (var sp = 0; sp < srcParams.length; sp++) {
            var t = "";
            try { t = _getMogrtText(srcParams[sp].param); } catch (eGt) { t = ""; }
            if (t) parts.push(t);
          }
    ...
    if (!params.mogrtMode) {
      try {
        ...
        if (detectComp && detectComp.properties) {
          var detectCaps = _detectCapabilities(detectComp.properties);
      ...
      for (var pi = 0; pi < rebuildList.length; pi++) {
        ...
        if (mgtComp && mgtComp.properties) {
          ...
          if (exactTextParam && s.text) {
            _setMogrtText(exactTextParam, s.text);
          } else if (s.text) {
            var tParams = _findAllTextParams(mgtComp.properties);
            if (tParams.length > 0) _setMogrtText(tParams[0].param, s.text);
          }
          var pParam = null;
    ...
    // ── DIRECTION B: NEW = generic ───────────────────────────────────────────
      ...
      if (mgtComp2 && mgtComp2.properties) {
        ...
        tParams2 = _findAllTextParams(mgtComp2.properties);
      }

      var newWordCount = wordsForXmp.length;
      var newNumInputs = tParams2.length;
      var newDist = _distributeWords(newWordCount, newNumInputs);
      ...
      for (var iii = 0; iii < newNumInputs; iii++) {
        ...
        try { _setMogrtText(tParams2[iii].param, pieces2.join(" ")); } catch (eS2) {}
      }

      // Persist XMP timings on the new clip's projectItem.
      try {
        _writeWordTimings(placedClip2.projectItem, {
          words:          wordsForXmp,
